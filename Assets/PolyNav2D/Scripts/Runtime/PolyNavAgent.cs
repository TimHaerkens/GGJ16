...
public class PolyNavAgent : MonoBehaviour{
    ...
    public Vector2 staticMovingDirection = new Vector2(0.93f, -0.35f);
    ...
    {
        SetTarget(new Vector2(go.transform.position.x - transform.position.x, go.transform.position.y - transform.position.y));
    }

    //set the target for the agent
    public bool SetTarget(Vector2 tg)
    {
	...
	void LateUpdate(){
        ...
        {
            walking = GetComponent<Animal>().walking;
            float speed = Mathf.Sqrt((Mathf.Abs(velocity.x) * Mathf.Abs(velocity.x)) + (Mathf.Abs(velocity.y) * Mathf.Abs(velocity.y)));
            if (speed > 0.1f) staticMovingDirection = movingDirection;

            if (!walking && hasTarget)
            {
                lookDirection = target;
            }
            else
            {
                lookDirection = staticMovingDirection;
            }
            ...
            GetComponent<Animal>().walkDirection = newZ;
        }
