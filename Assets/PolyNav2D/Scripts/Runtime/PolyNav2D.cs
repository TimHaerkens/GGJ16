...
public class PolyNav2D : MonoBehaviour {
	...
	///The layer mask of the obstacles
	...
	void TryNextFindPath(){
		...
		StartCoroutine( AStar.CalculatePath(startNode, endNode, nodes, RequestDone) );
	}
	...
	void CreatePolyMap(bool generateMaster){
		...
		for (int i = 0; i < navObstacles.Count; i++){
			...
			var points = InflatePolygon(obstacle.points, Mathf.Max(0.01f, inflateRadius + obstacle.extraOffset));
	...
	public bool CheckLOS (Vector2 posA, Vector2 posB){
		...
		if ( (posA - posB).sqrMagnitude < Mathf.Epsilon )
			return true;
/*
		if (Physics2D.CircleCast(posA, inflateRadius/2, (posB - posA).normalized, (posA - posB).magnitude, obstacleLayers.value))
			return false;
...
/*
		if (Physics2D.OverlapCircle(point, inflateRadius/2, obstacleLayers.value) != null)
			return false;
		return true;
*/
		for (int i = 0; i < map.allPolygons.Length; i++){
